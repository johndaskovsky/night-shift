# nightshift-doctor Specification

## Purpose
Defines the `/nightshift:doctor` skill, which verifies system dependencies (`qsv`, `flock`, `jq`) on demand and replaces the install-time dependency check formerly performed by `nightshift init`.
