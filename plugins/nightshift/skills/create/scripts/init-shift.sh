...
# Project bootstrap — idempotent. Ensures the project-layer scaffold exists
# on first /nightshift:create in a fresh project. Replaces the work that
# `nightshift init` used to do at install time.
...
.batch-manifest.json
GITIGNORE
  echo "Created .nightshift/.gitignore (project bootstrap)"
fi
